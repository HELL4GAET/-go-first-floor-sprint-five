...
func (r Running) Calories() float64 {
	...
	trainingDuration := r.Duration.Hours()
	res := ((CaloriesMeanSpeedMultiplier*avgSpeed + CaloriesMeanSpeedShift) * r.Weight / MInKm * trainingDuration * MinInHours)
	return res
}
...
func (w Walking) Calories() float64 {
	// вставьте ваш код ниже
	speed := math.Pow(w.meanSpeed(), 2) / (w.Height / 100)
	res := ((CaloriesWeightMultiplier*w.Weight + speed*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinInHours)
	return res
}
...
func (s Swimming) meanSpeed() float64 {
	...
	res := float64(s.LengthPool*s.CountPool/MInKm) / s.Duration.Hours()
	return res
}
...
func (s Swimming) Calories() float64 {
	// вставьте ваш код ниже
	speed := s.meanSpeed()
	res := (speed + SwimmingCaloriesMeanSpeedShift) * SwimmingCaloriesWeightMultiplier * s.Weight * s.Duration.Hours()
	return res
}
